...
double *complex_to_real_magnitude(fftw_complex *samples, uint64_t length) {
    double *out = (double *) malloc(sizeof(fftw_complex) * length);
    for (uint64_t i = 0; i < length; i++) {
        double re = samples[i][0];
...
void extract_freq_phase(double *samples, uint64_t length, uint64_t window_size,
    ...
    memcpy(freq, new_freq, window_size);
    free(new_freq);
